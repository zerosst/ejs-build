...
for row in $(jq -r '.[] | @base64' ../cores.json); do
    ...
    if [ "$listAllCores" = false ]; then
        ...
        #       不报错也不生效 —— 所以我连失败都看不见);
        #    ② 把证据写进**产物目录**($outputPath), 那个目录会随 artifact 回到我手里; 控制台拿不到。
        #    ③ 这里**不 exit**(免得把整个构建搞红): 真正的判据是 tools/verify_netplay_core.py
        #       解包看胶水里有没有符号 —— 那份报告决定"能不能打包"。
        DIAG="$outputPath/netplay-diag.txt"
        {
          echo "=== netplay 导出表补丁 ==="
          echo "cwd=$(pwd)"
          echo "buildPath=$buildPath"
          echo "--- find Makefile.emulatorjs ---"
          find "$buildPath" -maxdepth 6 -name 'Makefile.emulatorjs' 2>/dev/null
          echo "--- find 结果结束 ---"
        } >> "$DIAG"
        MKS=$(find "$buildPath" -maxdepth 6 -name 'Makefile.emulatorjs' 2>/dev/null | head -10)
        if [ -z "$MKS" ]; then
            echo "[netplay] ❌ 没找到 Makefile.emulatorjs → 导出表补不上, 单步入口不会导出" >> "$DIAG"
        else
            for MKF in $MKS; do
                if ! grep -q '_ejs_step_frames' "$MKF"; then
                    sed -i 's/_simulate_input,/_simulate_input,_ejs_step_frames,/' "$MKF"
                fi
                cnt=$(grep -c '_ejs_step_frames' "$MKF" || true)
                echo "[netplay] $MKF -> _ejs_step_frames x$cnt" >> "$DIAG"
                grep -n 'EXPORTED_FUNCTIONS =' "$MKF" | head -2 >> "$DIAG"
            done
        fi
        cat "$DIAG"
